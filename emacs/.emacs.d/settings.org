...
** TODO posframe terminal popup

It would be nice to have something like =shell-pop.el=, but in a floating child frame. Getting the popup to appear is easy enough, but unfortunately I think there's some focus selection issues with pgtk/wayland when returning to the parent.
...
At the expense of memory usage, improve performance on Windows.
...
trigger =completion-at-point=. Also, exclude commands not enabled for current mode from completion candidates.
...
Use eglot for lsp. Don't block when connecting to server. Close lsp server when last buffer in the project is killed. And, re-use existing server instance when jumping to definitions.
    ...
    (setq org-default-notes-file "~/notes/n.org"
          ...
          '(("n" "Note" entry (file+headline "~/notes/n.org" "Inbox")
            ...
            ("c" "Code Snippet" entry (file "~/notes/snippets.org")
             "* %?\t:%^{language}:\n#+BEGIN_SRC %\\1\n%i\n#+END_SRC")
            ("l" "Log Entry" entry (file+olp+datetree "~/notes/log.org")
             "* Week %<%V>: %?\n %T\n\n %i\n"
             :empty-lines 1
             :tree-type month)
...
#+begin_src emacs-lisp
  (use-package eat
    ...
    (:states 'insert
             ...
             "<down>" 'eat-line-next-matching-input-from-input))
#+end_src
...
Python essentially works out of the box with =python-ts-mode= mode and eglot
once =ruff= has been installed.
...
#+begin_src emacs-lisp
  ...
  (setq python-shell-interpreter-args "run --with ipython --with requests ipython")
...
#+begin_src emacs-lisp
  (use-package racket-mode
     ...
     (:keymaps 'racket-repl-mode-map
      "<up>" 'racket-repl-previous-input
      "<down>" 'racket-repl-next-input))
...
#+begin_src emacs-lisp
  (use-package copilot
    ...
    :config
    ;; Disable warnings about indentation and max characters
